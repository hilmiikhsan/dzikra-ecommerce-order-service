...
# example : make goose-up
	@echo " >> Installing goose if not installed"
	@go install github.com/pressly/goose/v3/cmd/goose@latest
	@goose -dir db/migrations postgres "host=localhost user=postgres password=21012123op dbname=dzikra_order sslmode=disable" up

goose-down:
# example : make goose-down
	@echo " >> Installing goose if not installed"
	@go install github.com/pressly/goose/v3/cmd/goose@latest
	@goose -dir db/migrations postgres "host=localhost user=postgres password=21012123op dbname=dzikra_order sslmode=disable" down

goose-status:
# example : make goose-status
	@echo " >> Installing goose if not installed"
	@go install github.com/pressly/goose/v3/cmd/goose@latest
	@goose -dir db/migrations postgres "host=localhost user=postgres password=21012123op dbname=dzikra_order sslmode=disable" status

seed:
# make seed total=10 table=roles
	$(GO_CMD) run $(MAIN) seed -total=$(total) -table=$(table)

PROTO_SRC_DIR := ./cmd/proto/transaction
PROTO_OUT_DIR := ./cmd/proto/transaction
